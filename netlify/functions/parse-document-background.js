...
exports.handler = async (event) => {
  ...
  try {
    ...
    await store.setJSON(jobId, { status: 'processing', createdAt: new Date().toISOString() });
...
Document filename: ${fileName}`;
    ...
    if (isPDF) {
      ...
      if (!textContent || textContent.trim().length === 0) {
        await store.setJSON(jobId, { status: 'error', error: 'Could not extract text from file.' });
        return;
      }
    ...
    try {
      ...
      if (first === -1 || last <= first) throw new Error('No JSON found');
      parsedData = JSON.parse(raw.substring(first, last + 1));
    } catch (e) {
      parsedData = {
        documentType: 'Unknown', documentCategory: 'Other',
        summary: 'Response could not be parsed.', confidence: 'Low',
    ...
    if (jobId) {
      try {
        const store = getJobStore();
        await store.setJSON(jobId, { status: 'error', error: err.message || 'Internal server error' });
      } catch {}
    }
  }
};
