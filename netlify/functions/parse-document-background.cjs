...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!jobId || !fileContent || !fileName) {
      // Nothing useful to do — background functions can't return errors to the client
      console.error('Missing jobId, fileContent, or fileName');
      return;
    }

    // Mark job as processing immediately so the poller knows it started
    await supabase.from('parse_jobs').upsert({
      id: jobId,
      status: 'processing',
    ...
    if (!textContent || textContent.trim().length === 0) {
      await supabase.from('parse_jobs').upsert({
        id: jobId,
        status: 'error',
        error: 'Could not extract any text from this file. It may be an image-only scan or encrypted.',
      });
      return;
    }
...

{
  ...
  "customFields": {
    "Use this object for any data important and specific to this document type but not covered above. For insurance: policyNumber, coverageLimits, etc. For court rulings: caseNumber, jurisdiction, verdict. For medical: diagnoses, medications. For scientific papers: methodology, findings, citations."
  }
}
...
${textContent.substring(0, 60000)}`;
    ...
    try {
      ...
      parsedData = {
        ...
        summary: rawResponse.substring(0, 2000), confidence: 'Low',
        flags: [`Response parse error: ${parseErr.message}`],
    ...
    if (jobId) {
      ...
      await supabase2.from('parse_jobs').upsert({
        id: jobId,
        status: 'error',
